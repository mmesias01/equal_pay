...
capture confirm file "${full}/ENAHO_oax_pea.dta"
if !_rc {
    use "${full}/ENAHO_oax_pea.dta", clear
}
else {
    use "ENAHO_oax_pea.dta", clear
}
...
* 2) Muestra: PEA 18–65, categorías 1–4, ingreso_total y horas válidos
keep if inlist(p507,1,2,3,4)
keep if inrange(edad,18,65)
drop if missing(ingreso_m_total, horas_sem)
drop if ingreso_m_total<=0 | horas_sem<=0

* Dependiente
gen lnym = ln(ingreso_m_total)
